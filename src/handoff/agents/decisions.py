...
class SafetyEnsembleProvider:
    ...
    HAZARD_KEYWORDS = [
        "burning smell", "smells like it's burning", "smells like burning",
        "burning odor", "smells burnt", "smoke", "gas smell", "smell gas",
        "carbon monoxide", "sparked", "sparking", "pouring", "flooding",
        "sewage",
    ]
    ...
    def classify(self, raw_request: str, photo_descriptions: list[str]) -> TriageDecision:
        ...
        if isinstance(self.inner, HeuristicTriageProvider):
            ...
            scan_text = mask(text)
        # Chatter cap: device noun phrase present WITHOUT any activation cue
        # means the LLM's own emergency call is an over-read — cap at URGENT.
        # Genuine activation cues (went off / blaring / visible smoke / fire)
        # bypass this cap through the unmasked hazard-net scan below.
        if self._smoke_detector_chatter(text) and d.urgency == Urgency.EMERGENCY:
            d = type(d)(urgency=Urgency.URGENT, category=d.category,
                        confidence=d.confidence,
                        rationale=f"{d.rationale} | capped: detector chatter without activation cue")
            return d
